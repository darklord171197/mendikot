...
function TenChip({ card }) {
  const red = card.s === "h" || card.s === "d";
  return (
    <span className={"ten-chip" + (red ? " ten-red" : " ten-black")}>
      10{SUIT_SYMBOL[card.s]}
    </span>
  );
}
      ...
      <div className="result-modal">
        ...
        <div className={"result-header " + (myTeamWon ? "result-win" : "result-lose")}>
          ...
          <div className="result-headline">
            {myTeamWon ? "Yay! Your Team Won!" : "Nay! Your Team Lost"}
          </div>
          <div className="result-subline">
            {isGameOver
              ? "Match over · Team " + (result.winner + 1) + " wins!"
              : result.bawanya
                ? "Bawanya — all tricks! +" + result.points + " pts"
                : result.mendikot
                  ? "Mendikot — all four tens! +" + result.points + " pts"
                  : "+" + result.points + " point"}
          </div>
        </div>
        ...
        <div className="result-teams">
          {[0, 1].map((t) => {
            const isMe = t === myTeam;
            const tens = result.capturedTens?.[t] || [];
            return (
              <div key={t} className={"result-team-col " + (isMe ? "rt-mine" : "rt-theirs")}>
                <div className="rt-label">
                  Team {t + 1}{isMe ? " · You" : ""}
                  {result.winner === t && <span className="rt-winner-badge">Winner</span>}
                </div>
                <div className="rt-stat-row">
                  <span className="rt-stat-label">Tricks</span>
                  <span className="rt-stat-val">{result.tricks[t]}</span>
                </div>
                <div className="rt-stat-row">
                  <span className="rt-stat-label">Tens</span>
                  <span className="rt-stat-val">{result.tens[t]}/4</span>
                </div>
                {tens.length > 0 ? (
                  <div className="rt-tens-row">
                    {tens.map((c) => <TenChip key={c.id} card={c} />)}
                  </div>
                ) : (
                  <div className="rt-tens-row rt-no-tens">—</div>
                )}
                <div className="rt-points">
                  {result.winner === t ? "+" + result.points + " pts" : "+0 pts"}
                </div>
              </div>
            );
          })}
        ...
        <div className="result-standings">
          ...
          <div className="result-standing-row">
            {[0, 1].map((t) => (
              <div key={t} className={"rs-team " + (t === myTeam ? "rs-mine" : "rs-theirs")}>
                <div className="rs-name">Team {t + 1}{t === myTeam ? " (You)" : ""}</div>
                <div className="rs-score">{result.scores[t]} pts</div>
                <div className="rs-stats">
                  <span title="Normal wins">W {result.normalWins?.[t] ?? 0}</span>
                  <span title="Mendikots">M {result.mendikotCount?.[t] ?? 0}</span>
                  <span title="Bawanyas">B {result.bawanyaCount?.[t] ?? 0}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="result-actions">
          {isHost ? (
            <button
              className="start-btn result-next-btn"
              onClick={isGameOver ? onPlayAgain : onNextRound}
            >
              {isGameOver ? "Play Again" : "Next Round"}
            </button>
          ) : (
            <p className="result-wait">Waiting for host to continue...</p>
          )}
        </div>
      </div>
    </div>
...
export default function RoomPage() {
  ...
  const seatSlots6 = ["bottom", "bottom-left", "top-left", "top", "top-right", "bottom-right"];
  const seatFor = (idx) => {
  ...
  const copyInvite = async () => {
    const text = window.location.origin + "/room/" + code + "/lobby";
    try {
  ...
  for (let i = 0; i < state.players.length; i++) {
    ...
    const seatClass =
      "seat seat-" + slot +
      (ally ? " ally" : " opp") + (me ? " me" : "") +
      (active ? " active" : "") + (p.connected ? " online" : " offline");
    ...
    <div className="game-page">
      ...
      <div className="scoreboard">
        <div className={"team-card " + (myTeam === 0 ? "mine" : "theirs")}>
          <div className="team-label">Team 1{myTeam === 0 ? " · You" : ""}</div>
          <div className="team-score">{state.scores?.[0] ?? 0}</div>
          <div className="team-round">
            <span className="tr-item">
              <span className="tr-key">Tricks</span>
              <span className="tr-val">{state.tricksWon?.[0] ?? 0}</span>
            </span>
            <span className="tr-item">
              <span className="tr-key">Tens</span>
              <span className="tr-val">{state.tensWon?.[0] ?? 0}/4</span>
            </span>
          </div>
          {ct0.length > 0 && (
            <div className="captured-tens">
              {ct0.map((c) => <TenChip key={c.id} card={c} />)}
            </div>
          )}
          <div className="team-match-stats">
            <span title="Normal wins">W:{state.normalWins?.[0] ?? 0}</span>
            <span title="Mendikots">M:{state.mendikotCount?.[0] ?? 0}</span>
            <span title="Bawanyas">B:{state.bawanyaCount?.[0] ?? 0}</span>
          </div>
        </div>
        ...
        <div className={"team-card " + (myTeam === 1 ? "mine" : "theirs")}>
          <div className="team-label">Team 2{myTeam === 1 ? " · You" : ""}</div>
          <div className="team-score">{state.scores?.[1] ?? 0}</div>
          <div className="team-round">
            <span className="tr-item">
              <span className="tr-key">Tricks</span>
              <span className="tr-val">{state.tricksWon?.[1] ?? 0}</span>
            </span>
            <span className="tr-item">
              <span className="tr-key">Tens</span>
              <span className="tr-val">{state.tensWon?.[1] ?? 0}/4</span>
            </span>
          </div>
          {ct1.length > 0 && (
            <div className="captured-tens">
              {ct1.map((c) => <TenChip key={c.id} card={c} />)}
            </div>
          )}
          <div className="team-match-stats">
            <span title="Normal wins">W:{state.normalWins?.[1] ?? 0}</span>
            <span title="Mendikots">M:{state.mendikotCount?.[1] ?? 0}</span>
            <span title="Bawanyas">B:{state.bawanyaCount?.[1] ?? 0}</span>
          </div>
        </div>
      </div>
        ...
        <div className={"felt seats-" + state.playerCount}>
          ...
          <div className="table-center">
            ...
            {state.status === "playing" && state.trick.length === 0 && (
              <div className="hint">Lead a card</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bottom-panel">
        <div className="host-actions">
          ...
          {state.isHost && state.status === "gameOver" && (
            <button className="start-btn" onClick={nextRound}>Play Again</button>
          )}
        </div>
        <h3 className="hand-title">
          Your Hand — {state.myHand.length} cards
          {myTurn && state.status === "playing" && <span className="your-turn"> · Your Turn</span>}
        </h3>
            ...
            state.myHand.map((card) => {
              ...
              return (
                <CardFace key={card.id} card={card}
                  mode={playable ? "playable" : "hand"} size="lg"
          ...
          <div className="trump-modal">
            ...
            <div className="trump-options">
              {["s", "h", "d", "c"].map((suit) => {
                ...
                return (
                  <button key={suit} className={red ? "red" : "black"} onClick={() => chooseTrump(suit)}>
                    {SUIT_SYMBOL[suit]}
                  </button>
                );
              })}
            </div>
            <button className="cancel-btn" onClick={() => setPendingTrumpCard(null)}>Cancel</button>
          </div>
        </div>
      )}
    </div>
